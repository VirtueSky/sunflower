...
namespace VirtueSky.ControlPanel.Editor
{
    public class CPQHierarchyDrawer
    {
        ...
        {
            QSettings.getInstance().inited = true;
            QSettings.getInstance().isProSkin = EditorGUIUtility.isProSkin;
            QSettings.getInstance().separatorColor =
                QSettings.getInstance().isProSkin ? new Color(0.18f, 0.18f, 0.18f) : new Color(0.59f, 0.59f, 0.59f);
            QSettings.getInstance().yellowColor =
                QSettings.getInstance().isProSkin ? new Color(1.00f, 0.90f, 0.40f) : new Color(0.31f, 0.31f, 0.31f);
            QSettings.getInstance().checkBoxChecked = QResources.getInstance().getTexture(QTexture.QCheckBoxChecked);
            QSettings.getInstance().checkBoxUnchecked =
                QResources.getInstance().getTexture(QTexture.QCheckBoxUnchecked);
            QSettings.getInstance().restoreButtonTexture = QResources.getInstance().getTexture(QTexture.QRestoreButton);
            QSettings.getInstance().componentsOrderList = new QComponentsOrderList(window);
        }
        ...
        public static void OnGUI(Rect position, EditorWindow window)
        {
            ...
            if (!QSettings.getInstance().inited || QSettings.getInstance().isProSkin != EditorGUIUtility.isProSkin)
                init(window);

            QSettings.getInstance().indentLevel = 8;
            QSettings.getInstance().scrollPosition =
                EditorGUILayout.BeginScrollView(QSettings.getInstance().scrollPosition);
            {
                Rect targetRect = EditorGUILayout.GetControlRect(GUILayout.Height(0));
                if (Event.current.type == EventType.Repaint) QSettings.getInstance().totalWidth = targetRect.width + 8;

                QSettings.getInstance().lastRect = new Rect(0, 1, 0, 0);

                // COMPONENTS
                drawSection("COMPONENTS SETTINGS");
                float sectionStartY = QSettings.getInstance().lastRect.y + QSettings.getInstance().lastRect.height;
                ...
                drawLeftLine(sectionStartY,
                    QSettings.getInstance().lastRect.y + QSettings.getInstance().lastRect.height,
                    QSettings.getInstance().separatorColor);

                // ORDER
                drawSection("ORDER OF COMPONENTS");
                sectionStartY = QSettings.getInstance().lastRect.y + QSettings.getInstance().lastRect.height;
                drawSpace(8);
                ...
                drawLeftLine(sectionStartY,
                    QSettings.getInstance().lastRect.y + QSettings.getInstance().lastRect.height + 4,
                    QSettings.getInstance().separatorColor);
        ...
        private static void drawErrorComponentSettings()
        {
            if (drawComponentCheckBox("Error", QSetting.ErrorShow))
            {
                ...
                QSettings.getInstance().indentLevel += 16;
        ...
        private static void drawOrderSettings(Rect position, EditorWindow window)
        {
            ...
            QSettings.getInstance().indentLevel += 4;
            ...
            if (QSettings.getInstance().componentsOrderList == null)
                QSettings.getInstance().componentsOrderList = new QComponentsOrderList(window);
            QSettings.getInstance().componentsOrderList.draw(rect, componentIds);
        ...
        private static void drawSection(string title)
        {
            ...
            drawLeftLine(rect.y, rect.y + 24, QSettings.getInstance().yellowColor);

            rect.x = QSettings.getInstance().lastRect.x + 8;
            rect.y += 4;
        ...
        private static void drawSeparator(int spaceBefore = 0, int spaceAfter = 0, int height = 1)
        {
            ...
            rect.width += 8;
            EditorGUI.DrawRect(rect, QSettings.getInstance().separatorColor);
            if (spaceAfter > 0) drawSpace(spaceAfter);
        }

        private static bool drawComponentCheckBox(string label, QSetting setting)
        {
            QSettings.getInstance().indentLevel += 8;
            ...
            if (GUI.Button(rect,
                    isChecked ? QSettings.getInstance().checkBoxChecked : QSettings.getInstance().checkBoxUnchecked,
                    GUIStyle.none))
            {
        ...
        private static bool drawCheckBoxRight(string label, QSetting setting)
        {
        ...
        {
            EditorGUI.DrawRect(new Rect(x, y, width, height), QSettings.getInstance().separatorColor);
        }

        private static void drawLeftLine(float fromY, float toY, Color color, float width = 0)
        {
            EditorGUI.DrawRect(
                new Rect(0, fromY, width == 0 ? QSettings.getInstance().indentLevel : width, toY - fromY),
                color);
        }
            ...
            float remWidth = 0)
        {
            ...
            Rect rect = new Rect(QSettings.getInstance().indentLevel + addIndent,
                QSettings.getInstance().lastRect.y + QSettings.getInstance().lastRect.height,
                (width == 0
                    ? QSettings.getInstance().totalWidth - QSettings.getInstance().indentLevel - addIndent - remWidth
                    : width), height);
            QSettings.getInstance().lastRect = rect;
            return rect;
        }
        ...
        {
            if (GUI.Button(
                    new Rect(QSettings.getInstance().lastRect.x + QSettings.getInstance().lastRect.width - 16 - 5,
                        QSettings.getInstance().lastRect.y - 20, 16, 16),
                    QSettings.getInstance().restoreButtonTexture, GUIStyle.none))
            {
        ...
        private static void drawTextField(string label, QSetting setting)
        {
        ...
        private static bool drawFoldout(string label, QSetting setting)
        {
        ...
        private static void drawColorPicker(string label, QSetting setting)
        {
        ...
        private static Enum drawEnum(string label, QSetting setting, Type enumType)
        {
            Enum currentEnum =
        ...
        private static void drawIntSlider(string label, QSetting setting, int minValue, int maxValue)
        {
        ...
        private static void drawFloatSlider(string label, QSetting setting, float minValue, float maxValue)
        {
            Rect rect = getControlRect(0, 16, 34, 4);
