...
{
    public class AssetFinderSplitView
    {
        ...
        void RefreshSpliterPos(int index, float px)
        {
            ...
            var m = isHorz
                ...
                : (Event.current.mousePosition.y - r1.y);
        ...
        void DrawSpliter(int index, float px)
        {
            ...
            EditorGUIUtility.AddCursorRect(dRect2,
                isHorz ? MouseCursor.ResizeHorizontal : MouseCursor.ResizeVertical);
            if (Event.current.type == EventType.MouseDown &&
                dRect2.Contains(Event.current.mousePosition))
            {
                resizeIndex = index;
