...
{
    public class AssetFinderBookmark : IRefDraw
    {
        internal static HashSet<string> guidSet = new HashSet<string>();

        internal static HashSet<string>
            instSet =
                new HashSet<string>(); // Do not reference directly to SceneObject (which might be destroyed anytime)
        ...
        public AssetFinderBookmark(IWindow window)
        {
            this.window = window;
        ...
        void DrawButtons(Rect rect)
        {
            ...
            GUILayout.BeginArea(rect);
            {
                GUILayout.BeginHorizontal();
                {
                    ...
                    {
                        AssetFinderExport.ExportCSV(refs.Values.ToArray());
                    }

                    if (GUILayout.Button("Delete"))
                    {
                        AssetFinderUnity.BackupAndDeleteAssets(refs.Values.ToArray());
                        Clear();
                        GUIUtility.ExitGUI();
                    }
                }
        ...
        public void RefreshView()
        {
            ...
            {
                var asset = AssetFinderCache.Api.Get(guid, false);
            ...
            {
                refs.Add(instId,
                    new AssetFinderSceneRef(0,
                        EditorUtility.InstanceIDToObject(int.Parse(instId))));
            }
