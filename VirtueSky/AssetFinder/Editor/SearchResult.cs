...
namespace VirtueSky.AssetFinder.Editor
{
    ...
    public class SearchResult : IEnumerable<SearchResultGroup>, ISerializationCallbackReceiver
    {
        ...
        public SearchResultGroup this[int index]
        {
        ...
        public bool SearchCompletedSuccessfully
        {
            get { return success; }
        }

        public bool InitialSceneSetupConfigured
        {
            get { return initialSceneSetup != null && initialSceneSetup.Length > 0; }
        }
        ...
        public AssetUsageDetector.Parameters SearchParameters
        {
            get { return m_searchParameters; }
        }

        public SearchResult(bool success, List<SearchResultGroup> result, HashSet<Object> usedObjects, SceneSetup[] initialSceneSetup, AssetUsageDetector searchHandler,
        ...
        public void RefreshSearchResultGroup(SearchResultGroup searchResultGroup, bool noAssetDatabaseChanges)
        {
            ...
            if (searchResultGroup.Type == SearchResultGroup.GroupType.Scene && EditorApplication.isPlaying &&
                !EditorSceneManager.GetSceneByPath(searchResultGroup.ScenePath).isLoaded)
            {
                            ...
                            for (int i = unusedObjectsSearchResultGroup.NumberOfReferences - 1; i >= 0; i--)
                            {
                                ...
                                for (int j = 0; j < node.NumberOfOutgoingLinks; j++)
                                {
                                    if (node[j].targetNode.usedState ==
                                        ReferenceNode.UsedState.Used) // User has explicitly displayed this used child object/sub-asset in the TreeView
                                        continue;
    ...
    public class SearchResultGroup : IEnumerable<ReferenceNode>
    {
        ...
        public ReferenceNode this[int index]
        {
                ...
                else
                {
                    if (Type == GroupType.UnusedObjects)
                    {
                        if (searchResult != null && searchResult.HasPendingLazySceneSearchResults)
                            EditorGUILayout.HelpBox("Some scene(s) aren't searched yet (lazy scene search). Refreshing those scene(s) will automatically update this list.",
                                MessageType.Warning);
                    ...
                    if (treeView == null)
                    {
                        ...
                        treeView = new SearchResultTreeView(treeViewState, references,
                            (Type == GroupType.UnusedObjects) ? SearchResultTreeView.TreeType.UnusedObjects : SearchResultTreeView.TreeType.Normal,
                            searchResult != null ? searchResult.UsedObjects : null, searchResult != null && searchResult.SearchParameters.hideDuplicateRows,
                    ...
                    if (treeViewSearchField == null)
                    {
                        ...
                        treeViewSearchField.downOrUpArrowKeyPressed +=
                            () => treeView.SetFocusAndEnsureSelectedItem(); // Not assigning SetFocusAndEnsureSelectedItem directly in case treeView's value changes
                    }
                    ...
                    treeViewState.searchTerm =
                        treeViewSearchField.OnToolbarGUI(new Rect(searchFieldRect.x, searchFieldRect.y, searchFieldRect.width - 100f, searchFieldRect.height),
                            treeViewState.searchTerm) ?? "";
                    treeViewState.searchMode =
                        (SearchResultTreeView.SearchMode)EditorGUI.EnumPopup(new Rect(searchFieldRect.xMax - 100f, searchFieldRect.y, 100f, searchFieldRect.height),
                            treeViewState.searchMode);
                    if (treeViewState.searchTerm != previousSearchTerm || treeViewState.searchMode != previousSearchMode)
                    ...
                    else if ((ev.type == EventType.ValidateCommand || ev.type == EventType.ExecuteCommand) && ev.commandName == "Find" && treeView.HasFocus())
                    {
                        if (ev.type == EventType.ExecuteCommand)
                        {
                            ...
                            scrollPosition = FrameRectInScrollView(scrollPosition,
                                new Vector2(searchFieldRect.y - 2f, searchFieldRect.yMax + EditorGUIUtility.singleLineHeight + 2f), window.position.height);
                            window.Repaint();
                    ...
                    if (treeViewKeyboardNavigation && treeView.HasFocus() && treeView.HasSelection())
                    {
                        ...
                        if (treeView.GetRowRectWithId(treeViewState.lastClickedID, out targetTreeViewRowRect))
                        {
                            ...
                            {
                                if (isFirstRowSelected && (pressedKeyboardNavigationKey == KeyCode.UpArrow || pressedKeyboardNavigationKey == KeyCode.PageUp ||
                                ...
                                else if (isLastRowSelected && (pressedKeyboardNavigationKey == KeyCode.DownArrow || pressedKeyboardNavigationKey == KeyCode.PageDown ||
                ...
                if (obj)
                {
                    ...
                    switch (searchResultGroup.Type)
                    {
                        case SearchResultGroup.GroupType.Assets:
                            result.IsMainReference = obj.IsAsset() &&
                                                     (obj is GameObject || (obj.hideFlags & (HideFlags.HideInInspector | HideFlags.HideInHierarchy)) == HideFlags.None);
                            break;
