...
namespace VirtueSky.AssetFinder.Editor
{
    ...
    public partial class AssetUsageDetector
    {
        ...
        private readonly Dictionary<int, ReferenceNode>
            searchedUnityObjects = new Dictionary<int, ReferenceNode>(32768); // Unity objects use their instanceIDs as key which is more performant
        ...
        public SearchResult Run(Parameters searchParameters)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    if (searchInScenesInBuildTickedAll || (searchParameters.searchInScenes & SceneSearchMode.ScenesInBuildSettingsTickedOnly) ==
                ...
                if (searchParameters.searchInAssetsFolder && assetsToSearchSet.Count > 0)
                {
                    ...
                    foreach (string path in assetPaths)
                    {
                        if (searchParameters.showDetailedProgressBar && ++searchProgress % 30 == 1 &&
                            EditorUtility.DisplayCancelableProgressBar("Please wait...", "Searching assets", (float)searchProgress / searchTotalProgress))
                            throw new Exception("Search aborted");
                ...
                if (projectSettingsToSearch.Length > 0)
                {
                    ...
                    {
                        if (searchParameters.showDetailedProgressBar && ++searchProgress % 30 == 1 &&
                            EditorUtility.DisplayCancelableProgressBar("Please wait...", "Searching Project Settings", (float)searchProgress / searchTotalProgress))
                            throw new Exception("Search aborted");
                ...
                if (!string.IsNullOrEmpty(openPrefabStageAssetPath))
                {
                    ...
                    if (openPrefabStageContextObject != null && !openPrefabStageContextObject.Equals(null))
                    {
                        try
                        {
                            ...
                            MethodInfo prefabStageOpenerWithContext = typeof(PrefabStageUtility).GetMethod("OpenPrefab",
                                BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static, null, new Type[2] { typeof(string), typeof(GameObject) }, null);
        ...
        private void CalculateUnusedObjects(List<SearchResultGroup> searchResult, out HashSet<Object> usedObjectsSet)
        {
            ...
            foreach (KeyValuePair<string, ReferenceNode> kvPair in unusedMainObjectNodes)
            {
                ...
                if (unusedSubObjectNodes.TryGetValue(kvPair.Key, out unusedSubAssetNodesAtPath))
                {
                    ...
                    (!usedObjectPathsSet
                        .Contains(kvPair.Key)) // If a main asset has sub-assets and all of them are used, consider the main asset as used, as well (especially useful for Sprite assets)
                    currentSearchResultGroup.AddReference(kvPair.Value);
            ...
            foreach (KeyValuePair<string, List<ReferenceNode>> kvPair in
